    ...
    static void ProcessGameDataFromXML( TextAsset xmlString )
    {
        ...
        foreach (XmlElement element in elemList)
        {
            ...
            foreach (Mission mission in requestMissions)
            {
                    ...
                    if (element.GetAttribute("name") == "STAR1") star1 = 2;//int.Parse(element.GetAttribute("value"));
                    if (element.GetAttribute("name") == "STAR2") star2 = ((mission.amount / 2) + (mission.amount / 4));//int.Parse(element.GetAttribute("value"));
                    if (element.GetAttribute("name") == "STAR3") star3 = GetMission(mission.type).amount; //int.Parse(element.GetAttribute("value"));   
